...
public class DirectSender {
	...
	public void send(String context, int count) {
		...
		for (int i = 0; i < count; i++) {
			message = context+i;
			if (i % 2 == 0) {
				this.rabbitTemplate.convertAndSend(DirectRabbitConfig.exchangeName,"even",message);
				
				this.rabbitTemplate.send(DirectRabbitConfig.exchangeName, "even", new Message(message.getBytes(),new MessageProperties()));
				//发布模式
				...
				this.rabbitTemplate.convertAndSend(DirectRabbitConfig.exchangeName, "odd", message);
			}
